 ...
 */
GraphicsFactory.prototype._createContainer = function(type, parentGfx, element) {
  var outerGfx = parentGfx.group().attr('class', 'djs-group'),
      gfx = outerGfx.group().attr('class', 'djs-element djs-' + type);

  // create element type attribute
  var el = element.type.split(':');
  gfx.attr('data-element-type', el[el.length - 1]);
...
GraphicsFactory.prototype.create = function(type, element) {
  ...
  return this._createContainer(type, childrenGfx, element);
};
